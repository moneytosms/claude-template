...
1. Clone the template to a temp dir: `git clone https://github.com/moneytosms/claude-template <tmp>` (shallow).
2. Diff the temp `.claude/` baseline against this project's, focusing on:
   - `.claude/hooks/`, `.claude/agents/`, `.claude/rules/`, `.claude/commands/`, `.claude/output-styles/`,
     `.claude/statusline.ps1`, and the non-permission parts of `.claude/settings.json`.
...
5. Delete the temp clone. Report what was updated and what was skipped.
